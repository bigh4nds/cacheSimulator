...
{
	unsigned int address;
	unsigned int addressMax = 0xFFFFFFFF;
	int count = 0;
	int type = 0;
	...
	for(i=0; i<10000; i++)
	{
        double randScale = (double)rand()/RAND_MAX;
        address = (unsigned int) (addressMax * randScale);
		if((instCache == 1 && type != 2) || instCache == 0)
        {
            parseBitString(address);
            access(type, address);
...
void displayInstResults()
{
    ...
    if(iMiss+iHit > 0)
        ...
        printf("Instruction Cache Miss rate: 0%%\n");
}
